...
struct ContentView: View {
        ...
        NavigationView {
            List {
                ForEach(restaurants.sorted(by: self.settingStore.displayOrder.predicate())) { restaurant in
                    if self.showRestaurant(restaurant: restaurant) {
    ...
    private func showRestaurant(restaurant:Restaurant)->Bool{
        return (restaurant.priceLevel <= self.settingStore.maxPriceLevel) && (restaurant.isCheckIn || !self.settingStore.showCheckInOnly)
    }
